...
use crate::model::message::{
    CitationMessage, CitationMessageAuthor, CitationSourceMessage, MESSAGE_LINK_REGEX,
    SKIP_MESSAGE_LINK_REGEX,
};
use serenity::all::{ChannelType, Context, CreateAllowedMentions, CreateMessage, Message, Ready};
...
impl EventHandler for BabyriteHandler {
    async fn message(&self, ctx: Context, message: Message) {
        ...
        let source_message = CitationSourceMessage::builder()
            .message(
                CitationMessage::builder()
                    .id(target_message.id)
                    .kind(target_message.kind)
        ...
        let embed = source_message.message.to_embed();
